...
opt-3.9 -load $INPUT_DEP_PATH/libInputDependency.so -load $UTILS_LIB -load $SC_PATH/libSCPass.so -load $OH_LIB/liboblivious-hashing.so $bitcode -sc -connectivity=1  -load-checkers-network="intresting_network" -dump-checkers-network="network_file" -oh-insert -num-hash 1 -o out.bc
