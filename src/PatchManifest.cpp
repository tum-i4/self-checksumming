...
void PatchManifest::readPatchManifest(std::string manifestFilePath){
	...
	for (const Json::Value& patch : root){
		this->size_patches[patch["size_placeholder"].asInt()]=patch["size_target"].asInt();
		this->address_patches[patch["add_placeholder"].asInt()]=patch["add_target"].asInt();
		this->hash_patches[patch["hash_placeholder"].asInt()]=patch["hash_target"].asInt();
	}
